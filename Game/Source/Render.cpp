...
// Blit to screen
bool Render::DrawTexture(SDL_Texture* texture, int x, int y, const SDL_Rect* section, bool camera, float speed, double angle, int pivotX, int pivotY) const
{
	bool ret = true;
	...
	SDL_Rect rect;
	if (camera) {
		rect.x = (int)(this->camera.x * speed) + x * scale;
		rect.y = (int)(this->camera.y * speed) + y * scale;
	}
	else {
		rect.x = x * scale;
		rect.y = y * scale;
